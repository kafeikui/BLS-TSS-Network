...
mod tests {
    ...
    async fn wait_for_event(
        ...
        let received_event = timeout(timeout_duration, receiver.recv())
            .await
    ...
    async fn assert_no_event_received(
        ...
        assert!(
            ...
            "Unexpectedly received an event when none was expected"
        );
    }
    ...
    async fn test_node_activation_listener() -> NodeResult<()> {
        ...
        env.register_node(false)
            .await
            .map_err(|e| anyhow!("Failed to register node: {}", e))?;
        env.set_node_state(false)
            .await
        ...
        assert!(
            listener.node_registry_address.is_some(),
            "Node registry address should be set after initialization"
        );
        assert_eq!(
            listener.node_registry_address.unwrap(),
            env.node_registry_address,
            "Node registry address does not match"
        );

        println!("Testing direct publishing through listener");
        listener
            .publish(NodeActivation {
