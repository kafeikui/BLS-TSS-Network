...
contract CreateAndSetOPStackChainMessengerScript is Deployer {
    ...
    function run() external {
        ...
        OPStackChainMessenger opStackChainMessenger;

        controllerRelayer = ControllerRelayer(_controllerRelayer);

        vm.broadcast(_deployerPrivateKey);
        opStackChainMessenger =
            new OPStackChainMessenger(_controllerRelayer, _opControllerOracleAddress, _opL1CrossDomainMessengerAddress);

        vm.broadcast(_deployerPrivateKey);
        controllerRelayer.setChainMessenger(_opChainId, address(opStackChainMessenger));
    }
}
