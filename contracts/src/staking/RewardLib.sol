...
library RewardLib {
    ...
    function _initialize(
        ...
        _updateDuration(reward, maxPoolSize, 0, rate, minRewardDuration, availableReward, 0);

        emit RewardInitialized(rate, availableReward, reward.startTimestamp, reward.endTimestamp);
    }
    ...
    function _accumulateBaseRewards(Reward storage reward) internal {
        ...
        reward.base.cumulativePerMicroLINK +=
            (uint256(reward.base.rate) * (cappedTimestamp - uint256(reward.base.lastAccumulateTimestamp)))._toUint96();
    ...
    function _accumulateDelegationRewards(Reward storage reward, uint256 delegatedAmount) internal {
        reward.delegated.cumulativePerDelegate = _calculateAccruedDelegatedRewards(reward, delegatedAmount)._toUint96();
    ...
    function _calculateReward(Reward storage reward, uint256 amount, uint256 duration)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculateAccruedDelegatedRewards(Reward storage reward, uint256 totalDelegatedAmount)
        internal
        view
        returns (uint256)
    {
        ...
        return uint256(reward.delegated.cumulativePerDelegate)
            + _calculateReward(reward, totalDelegatedAmount, elapsedDurationSinceLastAccumulate)
    ...
    function _calculateAccruedBaseRewards(Reward storage reward, uint256 amount) internal view returns (uint256) {
        uint256 elapsedDurationSinceLastAccumulate = _isDepleted(reward)
            ...
            amount
                * (
                    uint256(reward.base.cumulativePerMicroLINK)
                        + uint256(reward.base.rate) * elapsedDurationSinceLastAccumulate
                )
        ) / REWARD_PRECISION;
    }
    ...
    /// variables.
    function _calculateConcludedBaseRewards(Reward storage reward, uint256 amount, address staker)
        internal
        view
        returns (uint256)
    {
        return (amount * uint256(reward.base.cumulativePerMicroLINK)) / REWARD_PRECISION
    ...
    function _updateReservedRewards(
        ...
        uint96 deltaBaseReward = _calculateReward(reward, baseRewardAmount, duration)._toUint96();
        uint96 deltaDelegatedReward = _calculateReward(reward, delegatedRewardAmount, duration)._toUint96();
    ...
    /// @param delegatedRewardAmount The amount of delegated rewards to reserve
    /// or unreserve for
    function _reserve(Reward storage reward, uint256 baseRewardAmount, uint256 delegatedRewardAmount) internal {
        _updateReservedRewards(reward, baseRewardAmount, delegatedRewardAmount, true);
    }
    ...
    /// @param delegatedRewardAmount The amount of delegated rewards to reserve
    /// or unreserve for
    function _unreserve(Reward storage reward, uint256 baseRewardAmount, uint256 delegatedRewardAmount) internal {
        _updateReservedRewards(reward, baseRewardAmount, delegatedRewardAmount, false);
    }
    ...
    function _updateDuration(
        ...
        uint256 earnedBaseRewards = _getEarnedBaseRewards(reward, totalStakedAmount, totalDelegatedAmount);
        uint256 earnedDelegationRewards = _getEarnedDelegationRewards(reward, totalDelegatedAmount);

        uint256 remainingRewards = availableReward - earnedBaseRewards - earnedDelegationRewards;

        if (newRate != uint256(reward.base.rate)) {
            reward.base.rate = newRate._toUint80();
        }

        uint256 availableRewardDuration = (remainingRewards * REWARD_PRECISION) / (newRate * maxPoolSize);
        ...
        reward.reserved.base = (
            earnedBaseRewards
                + _calculateReward(reward, totalStakedAmount - totalDelegatedAmount, availableRewardDuration)
        )
            // Future base rewards for currently staked amounts based on the new duration
            ._toUint96();

        reward.reserved.delegated = (
            earnedDelegationRewards + _calculateReward(reward, totalDelegatedAmount, availableRewardDuration)
        )
    ...
    function _getEarnedBaseRewards(Reward storage reward, uint256 totalStakedAmount, uint256 totalDelegatedAmount)
        internal
        view
        returns (uint256)
    {
        return reward.reserved.base
            - _calculateReward(reward, totalStakedAmount - totalDelegatedAmount, _getRemainingDuration(reward));
    }

    /// @return The total amount of delegated rewards earned by all node operators
    function _getEarnedDelegationRewards(Reward storage reward, uint256 totalDelegatedAmount)
        internal
        view
        returns (uint256)
    {
        return reward.reserved.delegated - _calculateReward(reward, totalDelegatedAmount, _getRemainingDuration(reward));
    }
    ...
    function _slashOnFeedOperators(
        ...
        uint256 slashableBaseRewards = _getSlashableBaseRewards(reward, minOperatorStakeAmount, slashableDuration);
        uint256 slashableDelegatedRewards =
            _getSlashableDelegatedRewards(reward, slashableDuration, totalDelegatedAmount);
      ...
      feedOperators.length
        ...
        for (uint256 i; i < feedOperators.length; i++) {
            ...
            slashedBaseAmounts[i] =
                _slashOperatorBaseRewards(reward, slashableBaseRewards, operator, operatorStakedAmount);
            slashedDelegatedAmounts[i] =
                _slashOperatorDelegatedRewards(reward, slashableDelegatedRewards, operator, totalDelegatedAmount);
        ...
        emit RewardSlashed(feedOperators, slashedBaseAmounts, slashedDelegatedAmounts);
    }
    ...
    function _getSlashableBaseRewards(Reward storage reward, uint256 minOperatorStakeAmount, uint256 slashableDuration)
        private
        view
        returns (uint256)
    {
    ...
    function _getSlashableDelegatedRewards(
        ...
        return _calculateReward(reward, totalDelegatedAmount, slashableDuration)
    ...
    function _slashOperatorBaseRewards(
        ...
        uint256 earnedRewards = _getOperatorEarnedBaseRewards(reward, operator, operatorStakedAmount);
    ...
    function _slashOperatorDelegatedRewards(
        ...
        uint256 earnedRewards = _getOperatorEarnedDelegatedRewards(reward, operator, totalDelegatedAmount);
        uint256 slashedRewards = Math.min(slashableRewards, earnedRewards); // max capped by earnings
    ...
    /// has earned.
    function _getOperatorEarnedBaseRewards(Reward storage reward, address operator, uint256 operatorStakedAmount)
        internal
        view
        returns (uint256)
    {
        return _calculateAccruedBaseRewards(reward, operatorStakedAmount) - uint256(reward.missed[operator].base);
    }

    /// @return The amount of delegated rewards an operator
    /// has earned.
    function _getOperatorEarnedDelegatedRewards(Reward storage reward, address operator, uint256 totalDelegatedAmount)
        internal
        view
        returns (uint256)
    {
        return
            _calculateAccruedDelegatedRewards(reward, totalDelegatedAmount) - uint256(reward.missed[operator].delegated);
    }
