    ...
    Pausable
{
    ...
    constructor(PoolConstructorParams memory params) {
        ...
        s_pool._setConfig(
            params.initialMaxPoolSize, params.initialMaxCommunityStakeAmount, params.initialMaxOperatorStakeAmount
        );
    ...
    function hasAccess(address staker, bytes32[] memory proof) external view override returns (bool) {
        ...
        return MerkleProof.verify(proof, s_merkleRoot, keccak256(abi.encode(staker)));
    }
    ...
    function setPoolConfig(uint256 maxPoolSize, uint256 maxCommunityStakeAmount, uint256 maxOperatorStakeAmount)
        external
    ...
    /// @inheritdoc IStakingOwner
    function start(uint256 amount, uint256 initialRewardRate) external override(IStakingOwner) onlyOwner {
        ...
        s_reward._initialize(
            uint256(s_pool.limits.maxPoolSize), initialRewardRate, i_minRewardDuration, getAvailableReward()
        );
    }

    /// @inheritdoc IStakingOwner
    function conclude() external override(IStakingOwner) onlyOwner whenActive {
        s_reward._release(s_pool._getTotalStakedAmount(), getTotalDelegatedAmount());
    ...
    function withdrawUnusedReward() external override(IStakingOwner) onlyOwner whenInactive {
        uint256 unusedRewards =
            getAvailableReward() - uint256(s_reward.reserved.base) - uint256(s_reward.reserved.delegated);
    ...
    /// @inheritdoc IStakingOwner
    function removeOperators(address[] calldata operators) external override(IStakingOwner) onlyOwner whenActive {
        ...
        for (uint256 i; i < operators.length; i++) {
            ...
            if (principal > 0) {
                ...
                s_reward.reserved.base -=
                    s_reward._calculateReward(principal, s_reward._getRemainingDuration())._toUint96();
                ...
                // other operators.
                s_reward.reserved.delegated -= getDelegationReward(operator)._toUint96();
    ...
    function migrate(bytes calldata data) external override(IMigratable) whenInactive {
        ...
        i_LINK.transferAndCall(
            s_migrationTarget, uint256(amount + baseReward + delegationReward), abi.encode(msg.sender, data)
        );
    }
    ...
    function raiseAlert() external override(IAlertsController) whenActive {
        ...
        (uint256 roundId,,, uint256 lastFeedUpdatedAt,) = i_monitoredFeed.latestRoundData();
        ...
        bool isInPriorityPeriod = block.timestamp < lastFeedUpdatedAt + i_regularPeriodThreshold;
        ...
        uint256 rewardAmount = _calculateAlertingRewardAmount(stakedAmount, isInPriorityPeriod);
    ...
    function canAlert(address alerter) external view override(IAlertsController) returns (bool) {
        ...
        (uint256 roundId,,, uint256 updatedAt,) = i_monitoredFeed.latestRoundData();
    ...
    function getOperatorLimits() external view override(IStaking) returns (uint256, uint256) {
        return (i_minOperatorStakeAmount, uint256(s_pool.limits.maxOperatorStakeAmount));
    }
    ...
    function getAvailableReward() public view override(IStaking) returns (uint256) {
        return i_LINK.balanceOf(address(this)) - s_pool._getTotalStakedAmount() - s_pool.totalOperatorRemovedAmount;
    }
    ...
    function getDelegationReward(address staker) public view override(IStaking) returns (uint256) {
        ...
        return s_reward._getOperatorEarnedDelegatedRewards(staker, getTotalDelegatedAmount());
    }
    ...
    function getEarnedBaseRewards() external view override(IStaking) returns (uint256) {
        return s_reward._getEarnedBaseRewards(s_pool._getTotalStakedAmount(), getTotalDelegatedAmount());
    }
     ...
     */
    function onTokenTransfer(address sender, uint256 amount, bytes memory data)
        external
        validateFromLINK
        whenNotPaused
        whenActive
    {
        ...
        if (s_pool._isOperator(sender)) {
            ...
            if (s_merkleRoot != bytes32(0)) {
                ...
                if (!MerkleProof.verify(abi.decode(data, (bytes32[])), s_merkleRoot, keccak256(abi.encode(sender)))) {
                    revert AccessForbidden();
                }
            }
    ...
    function _stakeAsCommunityStaker(address staker, uint256 amount) private {
        ...
        s_reward.missed[staker].base += s_reward._calculateAccruedBaseRewards(extraNonDelegatedAmount)._toUint96();
        s_reward._reserve(extraNonDelegatedAmount, RewardLib._getDelegatedAmount(amount, i_delegationRateDenominator));
    ...
    function _stakeAsOperator(address staker, uint256 amount) private {
        ...
        // Check that the amount is less than or equal to the maximum allowed
        uint256 maxOperatorStakeAmount = uint256(s_pool.limits.maxOperatorStakeAmount);
        if (newStakedAmount > maxOperatorStakeAmount) {
            revert StakingPoolLib.ExcessiveStakeAmount(maxOperatorStakeAmount - currentStakedAmount);
        }
        ...
        s_reward.missed[staker].base += s_reward._calculateAccruedBaseRewards(amount)._toUint96();
    ...
    function _exit(address staker) private returns (uint256, uint256, uint256) {
        ...
        if (stakerAccount.isOperator) {
            ...
            uint256 baseReward = s_reward._calculateConcludedBaseRewards(stakerAccount.stakedAmount, staker);
            uint256 delegationReward =
                uint256(s_reward.delegated.cumulativePerDelegate) - uint256(s_reward.missed[staker].delegated);
            ...
            uint256 baseReward = s_reward._calculateConcludedBaseRewards(
                RewardLib._getNonDelegatedAmount(stakerAccount.stakedAmount, i_delegationRateDenominator), staker
            );
            delete s_pool.stakers[staker].stakedAmount;
            s_reward.reserved.base -= baseReward._toUint96();
    ...
    function _calculateAlertingRewardAmount(uint256 stakedAmount, bool isInPriorityPeriod)
        private
        view
        returns (uint256)
    {
        if (isInPriorityPeriod) return i_maxAlertingRewardAmount;
        return Math.min(stakedAmount / ALERTING_REWARD_STAKED_AMOUNT_DENOMINATOR, i_maxAlertingRewardAmount);
    }
