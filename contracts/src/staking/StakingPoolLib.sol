...
library StakingPoolLib {
    ...
    function _setConfig(
        ...
        if (maxPoolSize < (poolState.operatorsCount * maxOperatorStakeAmount) + poolState.totalCommunityStakedAmount) {
            revert InvalidMaxStakeAmount(maxOperatorStakeAmount);
        }
        if (pool.limits.maxPoolSize != maxPoolSize) {
    ...
    function _getRemainingPoolSpace(Pool storage pool) internal view returns (uint256) {
        StakingPoolLib.PoolState memory poolState = pool.state;
        return uint256(pool.limits.maxPoolSize)
            - (uint256(poolState.operatorsCount) * uint256(pool.limits.maxOperatorStakeAmount))
    ...
    function _addOperators(Pool storage pool, address[] calldata operators) internal {
        uint256 requiredReservedPoolSpace = operators.length * uint256(pool.limits.maxOperatorStakeAmount);
        uint256 remainingPoolSpace = _getRemainingPoolSpace(pool);
        if (requiredReservedPoolSpace > remainingPoolSpace) {
            revert InsufficientRemainingPoolSpace(remainingPoolSpace, requiredReservedPoolSpace);
        }
